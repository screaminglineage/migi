...
typedef struct {
    ...
    HashmapHashFn hash_fn;          // custom hash function (not used if NULL)
    HashmapEqFn eq_fn;              // custom equality function (not used if NULL)
} HashmapHeader;
...
#define hashmap_init(arena, hashmap)                                        \
        hm_init_impl(&(hashmap)->h, _hashmap_key_type((hashmap)->keys[0])), \
...
#define hashmap_pop(hashmap, k)                                                                 \
     (hm_delete_impl(&(hashmap)->h,                                                             \
        (hashmap)->keys, sizeof((hashmap)->keys[0]), _addr_of((k))),                            \
    (hashmap)->h.temp_index == 0                                                                \
      ? (hashmap)->values[0]                                                                    \
...
static void hm_init_impl(HashmapHeader *header, HashmapKeyType key_type) {
    if (header->hash_fn == NULL) {
        switch (key_type) {
            case HashmapKey_Str:    header->hash_fn = hash_fnv_str;   break;
            case HashmapKey_CStr:   header->hash_fn = hash_fnv_cstr;  break;
            case HashmapKey_Other:  header->hash_fn = hash_fnv_bytes; break;
            default:                migi_unreachable();               break;
        }
    }
    if (header->eq_fn == NULL) {
        switch (key_type) {
            case HashmapKey_Str:    header->eq_fn = eq_str;   break;
            case HashmapKey_CStr:   header->eq_fn = eq_cstr;  break;
            case HashmapKey_Other:  header->eq_fn = eq_bytes; break;
            default:                migi_unreachable();       break;
        }
    }
}
...
static HashmapItem hm_internal_index(HashmapHeader *header, void *keys, size_t key_size, void *search_key) {
    TIME_FUNCTION;
    HashmapItem result = {0};
    result.hash = header->hash_fn(search_key, key_size);
    ...
    while (true) {
        ...
        if (header->eq_fn(search_key, map_key, key_size)) {
...
static void *hm_put_impl(Arena *a, HashmapHeader *header,
    ...
    HashmapItem item = hm_internal_index(header, keys, key_size, new_key);
    if (!item.is_present) {
...
static void hm_delete_impl(HashmapHeader *header, void *keys, size_t key_size, void *delete_key) {
    TIME_FUNCTION;
    if (header->capacity == 0) {
        header->temp_index = 0;
        return;
    }
    HashmapItem item = hm_internal_index(header, keys, key_size, delete_key);
    if (!item.is_present) {
        header->temp_index = 0;
        return;
    }
    ...
    HashmapItem last_item = hm_internal_index(header, keys, key_size, last_key);
...
static void hms_get_impl(HashmapHeader *header, void *keys, size_t key_size, void *search_key) {
    HashmapItem item = hm_internal_index(header, keys, key_size, search_key);
    if (item.is_present) {
        header->temp_index = header->entries[item.entry_index].index;
    } else {
        header->temp_index = 0;
