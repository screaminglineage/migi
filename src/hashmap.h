...
// TODO: try to use _Generic for strmap and hashmap
...
static void *hm_grow(Arena *a, HashmapHeader_ *header, void *data, size_t entry_size, size_t entry_align, size_t at_least) {
    TIME_FUNCTION;
    ...
    byte *new_data = arena_push_bytes(a, entry_size * (header->capacity + 1), entry_align);
...
static HashmapItem hm_internal_index(HashmapHeader_ *header, void *data, size_t entry_size,
                                      void *key, size_t key_size, size_t key_offset, HashmapKeyType key_type) {
    TIME_FUNCTION;
    HashmapItem result = {0};
    result.hash = hash_fnv(key, key_size);
    ...
    while (true) {
        ...
        byte *map_key = (byte *)data + (header->entries[i].index * entry_size) + key_offset;
...
static void *hm_put_impl(Arena *a, HashmapHeader_ *header, void *data, size_t entry_size, size_t entry_align,
                         void *key, size_t key_size, size_t key_offset, HashmapKeyType key_type) {
    TIME_FUNCTION;
    void *new_data = data;
    if (header->size >= (size_t)(header->capacity * HASHMAP_LOAD_FACTOR)) {
        new_data = hm_grow(a, header, data, entry_size, entry_align, 0);
    }

    HashmapItem item = hm_internal_index(header, data, entry_size, key, key_size, key_offset, key_type);
    if (!item.is_present) {
...
static void hm_delete_impl(HashmapHeader_ *header, void *data, size_t entry_size,
                           void *key, size_t key_size, size_t key_offset, HashmapKeyType key_type) {
    TIME_FUNCTION;
    ...
    byte *last_key = (byte *)data + (header->size * entry_size) + key_offset;
    HashmapItem last_item = {0};
    if (key_type == HashmapKey_String) {
        String *last_key_str = (String *)last_key;
        last_item = hm_internal_index(header, data, entry_size, (void *)last_key_str->data, last_key_str->length, key_offset, key_type);
    } else if (key_type == HashmapKey_Other) {
        last_item = hm_internal_index(header, data, entry_size, last_key, key_size, key_offset, key_type);
    } else {
        migi_unreachable();
    }
...
static void hm_get_pair_impl(HashmapHeader_ *header, void *data, size_t entry_size,
                             void *key, size_t key_size, size_t key_offset, HashmapKeyType key_type) {
    HashmapItem item = hm_internal_index(header, data, entry_size, key, key_size, key_offset, key_type);
    if (item.is_present) {
        header->temp_index = header->entries[item.entry_index].index;
...
#define _addr_of_key(k)                                   \
    (_Generic((k),                                        \
        String:  (void *)(((String *)_addr_of(k))->data), \
        default: _addr_of(k)))

#define _size_of_key(hashmap, k)                    \
    (_Generic((k),                                  \
        String:  (((String *)_addr_of(k)))->length, \
        default: sizeof((hashmap)->data->key)))

#define _type_of_key(k)             \
    (_Generic((k),                  \
...
#define hashmap_reserve(arena, hashmap, count)                      \
    ((hashmap)->data = hm_grow((arena), (HashmapHeader_*)(hashmap), \
        ...
        _Alignof(__typeof__((hashmap)->data[0])), (count)))
...
#define hashmap_put(arena, hashmap, k, v)                                                              \
    ((hashmap)->data = hm_put_impl((arena), (HashmapHeader_*)(hashmap),                                \
        (void *)(hashmap)->data, sizeof((hashmap)->data[0]), _Alignof(__typeof__((hashmap)->data[0])), \
        _addr_of_key((k)), _size_of_key((hashmap), (k)),                                                          \
        offsetof(__typeof__((hashmap)->data[0]), key), _type_of_key((k))),                             \
...
#define hashmap_index_of(hashmap, k)                                       \
    (hm_get_pair_impl((HashmapHeader_*)(hashmap),                          \
        (hashmap)->data, sizeof (*(hashmap)->data),                        \
        _addr_of_key((k)), _size_of_key((hashmap), (k)),                              \
        offsetof(__typeof__((hashmap)->data[0]), key), _type_of_key((k))), \
...
#define hashmap_pop(hashmap, k)                                                                     \
     (hm_delete_impl((HashmapHeader_ *)(hashmap),                                                   \
        ...
        offsetof(__typeof__((hashmap)->data[0]), key), _type_of_key((k))),                          \
    (hashmap)->temp_index == 0                                                                      \
      ? (hashmap)->data[0].value                                                                    \
      : ((hashmap)->data[(hashmap)->size + 2].value = (hashmap)->data[(hashmap)->temp_index].value, \
        (hashmap)->data[(hashmap)->temp_index] = (hashmap)->data[(hashmap)->size + 1],              \
        (hashmap)->data[(hashmap)->size + 2].value))
