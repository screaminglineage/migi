#ifndef MIGI_ARENA_NEW_H
#define MIGI_ARENA_NEW_H
...
#define arena_copy(arena, type, mem, length) \
    (type *)memcpy(arena_push_nonzero((arena), type, (length)), (mem), (length))
...
static Arena *arena__init(ArenaOptions opt) {
    ...
    size_t commit_size = align_up(opt.commit_size, OS_PAGE_SIZE);
...
static void *arena_realloc_bytes(Arena *arena, void *old, size_t old_size, size_t new_size, size_t align) {
    ...
    assertf(old_size <= arena->position, "%s: old_size is greater than arena length", __func__);
...
static void arena_rewind(Checkpoint checkpoint) {
    ...
    while (current != checkpoint.current) {
        Arena *temp = current;
        current = current->prev;
        memory_release(temp, temp->reserved);
    }
