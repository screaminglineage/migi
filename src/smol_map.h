...
// values should be put in a separate dynamic array. The hashing is done on
// the user side while calling any of these functions.
//
...
static void smol_put(Arena *arena, SmolMap *smol, uint64_t hash, uint64_t value, bool replace);

// Returns a value or the default value provided, if it doesn't exist
static uint64_t smol_get(SmolMap *smol, uint64_t key, uint64_t fallback);

// Looks up a value into the hashmap
// If an arena is provided (arena != 0) it will update the old value with the new one
static uint64_t smol_lookup(Arena *arena, SmolMap *smol, uint64_t key_hash, uint64_t value);
...
static void smol_put(Arena *arena, SmolMap *hashmap, uint64_t key_hash, uint64_t value, bool replace) {
    ...
    if (!replace) {
        avow(hashmap->hashes[index] == (uint64_t)-1, "smol_add: collision occured but replacement is prohibited");
    }
...
static uint64_t smol_get(SmolMap *hashmap, uint64_t key_hash, uint64_t fallback) {
...
static uint64_t smol_lookup(Arena *arena, SmolMap *hashmap, uint64_t key_hash, uint64_t value) {
