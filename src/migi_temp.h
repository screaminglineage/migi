...
#define temp_alloc(type, size) \
    ((type *)arena_push_bytes(&temp_allocator_global_arena, sizeof(type) * size, _Alignof(type)))

migi_printf_format(1, 2) static String temp_format(const char *fmt, ...);
static void temp_reset();
static ArenaCheckpoint temp_save();
static void temp_rewind(ArenaCheckpoint checkpoint);
...
static String temp_format(const char *fmt, ...) {
    ...
    if (actual > reserved) {
        arena_pop_current(&temp_allocator_global_arena, char, reserved);
        mem = arena_push(&temp_allocator_global_arena, char, actual);
        vsnprintf(mem, actual, fmt, args2);
    } else if (actual < reserved) {
        arena_pop_current(&temp_allocator_global_arena, char, abs_difference(actual, reserved));
    }
    // pop off the null terminator
    arena_pop_current(&temp_allocator_global_arena, char, 1);
...
static void temp_rewind(ArenaCheckpoint checkpoint) {
    arena_rewind(&temp_allocator_global_arena, checkpoint);
}
