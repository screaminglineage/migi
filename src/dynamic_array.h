...
do {                                                                                           \
    ...
    if (new_length > (arr)->capacity) {                                                        \
        ...
        if ((arr)->capacity == 0) {                                                            \
            (arr)->arena = (arr)->arena? (arr)->arena: arena_init(.type=Arena_Linear);         \
            if (new_length < DYNAMIC_ARRAY_INIT_CAP) (arr)->capacity = DYNAMIC_ARRAY_INIT_CAP; \
        } else {                                                                               \
            (arr)->capacity = next_power_of_two(new_length);                                   \
        }                                                                                      \
        (arr)->data = arena_realloc_bytes((arr)->arena, (arr)->data,                           \
...
#endif // defined(DYNAMIC_ARRAY_USE_ARENA)
